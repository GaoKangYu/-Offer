...
private:
  ...
  **8、智能指针与RAII**
  ...
  答：Resource Acquisition Is Initialization，是C++特有的资源管理方式。思路：构造函数申请空间，析构函数释放空间。

  浅拷贝（通常赋值都是）：没有开辟新空间，新增指针指向同一空间
	
  深拷贝（new）：开辟新空间，新增指针指向值一样的不同空间
	
  引用计数技术：增加资源计数器，记录资源使用的次数，当计数器减为0时，表示该资源可以被释放，使用指针来写，解决资源释放问题。
 
  写时拷贝：解决资源修改问题，发生共享资源改变的时候，需要对其资源进行重新的拷贝，这样改变的拷贝时的值，而不影响原有的对象中的共享资源。
	 
  智能指针：像指针；会对自己资源进行释放；
	
  `shared_ptr`:带引用计数的智能指针
	
  `unique_ptr`:相对于auto_ptr取消了拷贝构造和=运算符重载，增加了move用法（移动构造，明确了之前的直接赋值剪切操作`std::unique_ptr<int> uptr2 = std::move(uptr);`，但剪切完之后uptr还是无法使用），可以采用move的方法使用数组(`Ary.push_back(std::move(p))`，不加move编译不过)。
	
  `auto_ptr`（及其废除原因）：在c++11标准中被废除，使用unique_ptr代替，功能相同且安全性提升（没有浅拷贝，浅拷贝容易导致重复删除问题），增加了delete析构特性和对数组的支持。对象出了类的作用域自动析构，通过这一有限的机制，这个类模板将一个指针保存在auto_ptr对象中，拥有这个指针的所有权，当对象析构时，会在析构函数中自动的调用delete，释放保存的指针。正因如此，不能有两个auto_ptr对象拥有同一指针，因为可能在某个时机两者都会尝试析构这个内部指针。两个auto_ptr对象之间发生赋值（例如作为实参通过值拷贝传参给函数）操作时，内部指针所属权会发生改变，赋值的右者对象会丧失所有权，会被设为null指针。
	
  `weak_ptr`:避免shared_ptr循环引用导致的析构异常问题，所以通常为一个强指针与一个弱指针配合使用。
	
  **9、线程（thread）与同步**

  答：

  cpu多线程通过时间片划分实现，代码交替运行
  ...
  线程不同步（两个线程操作同一个资源的时候造成预期外结果的现象）的原因：因为时间切片的缘故，一句c++语言被翻译为多行汇编语言后，可能执行到了汇编的某一行就切到了另一个线程。
	
  解决方法：
	
  1、原子操作：线程在访问资源时能确保其他线程都不在同一时间内访问该资源。
  
  ++value -->  `InterlockedAdd((long*)&value, 1);`（windows api），使用`InterlockedAdd`可以使得++value是原子操作。
	
  2、临界区：是一个小代码段，在代码能够执行前，它必须独占对某些共享资源的访问权，在代码退出临界区之前，系统将不给想要访问相同资源的其他线程进行调度。（把功能代码包裹在`EnterCriticalSection(&cs)`;  ...   LeaveCriticalSection(&cs);其中cs是临界区对象CRITICAL_SECTION cs;定义为全局变量，使用前需要进行初始化InitializeCritialSection(&cs); ...  DeleteCritialSection(&cs);），上锁的时候尽量颗粒度小（上锁代码段的运行时间快）
...
```
  ...
  mtx.lock();//std::lock_guard<std::mutex> lg(mtx);
  ...
  mtx.unlock();//std::lock_guard<std::mutex> lg(mtx);
```
